...
function Photo(name, imageUrl){
  ...
  console.log('do we have clicks ', this.timesClicked);
  if(timesClicked){
...
var savedImageString = localStorage.getItem('savedImage');

if(savedImageString){
  //change from a string to objects that- dont yet know they are images
  var arrayOfNotYetImageObjects = JSON.parse(savedImageString);

  for(var j = 0; j < arrayOfNotYetImageObjects.length; j++){
    new Photo(
      arrayOfNotYetImageObjects[j].name,
      arrayOfNotYetImageObjects.imageUrl,
      arrayOfNotYetImageObjects[j].timesClicked);
  }
...
} // closes our if else condition so that our object being parsed from a string value can then be added back into our image's objects by way of our constructor function.
...
function imageWasClicked(event){
  ...
  if(totalClicks >= rounds){
    ...
    // class13 ----- This grabs our constructor function objects and makes them a string to be stored in local storage
    localStorage.setItem('savedImage', JSON.stringify(allImages));
